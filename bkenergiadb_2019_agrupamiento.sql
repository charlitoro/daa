...
select *, 'Votante' as votante, case when estrato_ba in ('1','2') then 'bajo'
				when estrato_ba in ('3','4') then 'medio'
				when estrato_ba in ('5','6') then 'alto'
			end nivel_est
from ciudadanos join votaciones on cedula_ciu=ciudadano_v join barrios on barrio_ciu=codigo_ba
where lower(nombre_par) like '%conservador%' or
	  lower(nombre_par) like '%liberal%' or
	  lower(nombre_par) like '%polo%democratico%';
-- ciudadanos que no votaron 
select *, 'Abstinente' as votante, case when estrato_ba in ('1','2') then 'bajo'
				when estrato_ba in ('3','4') then 'medio'
				when estrato_ba in ('5','6') then 'alto'
			end nivel_est
from ciudadanos join barrios on barrio_ciu=codigo_ba left join votaciones on cedula_ciu=ciudadano_v
where ciudadano_v is null;
-- numero de los ciudadanos y porcentaje de los estratos bajo
select count(*)
from (select *, 'Votante' as votante, case when estrato_ba in ('1','2') then 'bajo'
				when estrato_ba in ('3','4') then 'medio'
				when estrato_ba in ('5','6') then 'alto'
			end nivel_est
	  from ciudadanos join votaciones on cedula_ciu=ciudadano_v join barrios on barrio_ciu=codigo_ba;)
